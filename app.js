...
async function loadScope(){const current=++revision;unsubscribe?.();unsubscribe=null;selected=null;items=[];render();const key=scope;
 if(cloudEnabled&&user&&api){unsubscribe=api.onValue(api.ref(db,ROOT+'/galleries/'+user.uid),s=>{if(current!==revision)return;items=Object.values(s.val()||{});render();},()=>notice('Нет доступа к облачной галерее. Проверьте подключение и правила базы.'));}
 else{try{const list=await localRead(key);if(current===revision){items=list;render();}}catch{notice('Браузер не разрешил сохранение. Примерка доступна, галерея — нет.');}}
}
async function write(item){const key=scope,token=revision;if(cloudEnabled&&user&&api){await api.set(api.ref(db,ROOT+'/galleries/'+user.uid+'/'+item.id),item);}else{const next=[...items.filter(x=>x.id!==item.id),item];await localWrite(key,next);if(token===revision){items=next;render();}}}
async function remove(id){if(cloudEnabled&&user&&api)await api.remove(api.ref(db,ROOT+'/galleries/'+user.uid+'/'+id));else{const next=items.filter(x=>x.id!==id);await localWrite(scope,next);items=next;render();}if(selected===id)selected=null;}
async function compress(file){
 ...
 const c=document.createElement('canvas');let side=1280;for(let i=0;i<6;i++){const scale=Math.min(1,side/Math.max(im.width,im.height));c.width=Math.max(1,Math.round(im.width*scale));c.height=Math.max(1,Math.round(im.height*scale));const g=c.getContext('2d');g.fillStyle='#fff';g.fillRect(0,0,c.width,c.height);g.drawImage(im,0,0,c.width,c.height);const data=c.toDataURL('image/jpeg',.84-i*.055);if(data.length<=250000)return data;side=Math.round(side*.8);}throw Error('Фото слишком сложное для сохранения. Попробуйте уменьшить его.');}finally{URL.revokeObjectURL(url);}
}
